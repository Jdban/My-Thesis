...
The algorithm uses multiple properties of zebra stripe crosswalks as neural network parameters and the results are discussed. 
...
\todo[inline]{JASON - rearrange the photos to match each-other}
...
\todo[inline]{Dr. Seng - any suggestions for a better title?}
...
\todo[inline]{JASON - More in depth than section in background, and more explanation of how they work}
...
\todo[inline]{JASON - take some stuff from my background portion and make it more in depth}
...
\todo[inline]{JASON - history section with source}
...
\todo[inline]{JASON - add pic for feedforward basic}
\todo[inline]{JASON - add source}
The most basic example of a neural network is a feedforward neural network. This network allows information to move only forwards, from the inputs to the outputs. An example of a basic feedforward neural network is shown in figure \ref{}. A series of weights is assigned to every connection, and the inputs are fed through the network, multiplied by every weight, summed, and then fed through to the next layer until the output values are generated. To train a feedforward network, one must either manually adjust the weights, or come up with a mathematical formula to do so. 
...
\todo[inline]{Dr. Seng - Is this the right spot and/or format for this? Talking about the 11 neural network inputs and how they were chosen seems huge for this section. Are there any suggestions on where to put it or how to condense it?}
...
\chapter{Results}
\label{results}
\todo[inline]{JASON - Before paper is final, check all the charts and numbers are updated}
...
\todo[inline]{JASON - redo fig \ref{fig:Neural1png} in graphvis with correct values for updated NN}
\begin{figure}[H]
\begin{center}
...
\todo[inline]{JASON - Ensure numbers are up to date}
...
\todo[inline]{JASON - zebra vs hybrid make table work right}
...
\todo[inline]{DR. SENG - Any suggestions on this title?}
...
\subsubsection{Issues Found and Room for Improvement}
\todo[inline]{DR. SENG -  any suggestions for a better title?}
...
\todo[inline]{JASON - Add more to this section after some more experimentation}
...
As an experiment, the results looked promising. If a crosswalk was correctly identified, the bounding lines were more or less correct, and could easily be used to guide the user across the crosswalk. 
