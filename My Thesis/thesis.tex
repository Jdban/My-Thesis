...
\author{Jason Banich}
\degreemonth{April} \degreeyear{2016} \degree{Master of Science}
\defensemonth{April} \defenseyear{2016}
...
\todo[inline]{Needs at least one more}
...
Zebra crosswalks have many properties that could be used to help identify them, one example would be the lengths of the crosswalk's edges. The edges are well defined in the image, so they can be collected rather easily. Other properties include the fact that painted lines are typically homogeneous in the center in terms of pixel intensity. Zebra crosswalk stripes are also spaced a consistent distance apart. The lines in the crosswalk are all also exactly parallel. 
\todo[inline]{Needs more content}

\section{Coughlan Approach}
\todo[inline]{@Seng or @Jenee what do I do for paper title?}
...
\todo[inline]{HARDCORE RIGHT HERE - Talk about the 11 neural network inputs and how they were chosen.}
...
%Jeneeplz
For this project, neural networks are used for the prediction of striplets as either being part of a crosswalk or not. Neural networks require a set number of hidden layers with a set number of neurons, so some experimentation was done to determine these numbers. As shown in figure \ref{fig:Neural1png}, for this project, the final result was one hidden layer of 15 neurons. 
...
\todo[inline]{redo fig \ref{fig:Neural1png} in graphvis}
\begin{figure}[h!]
\begin{center}
...
\subsection{Crosswalk Detection}
%Jeneeplz
Once the neural network has done its job of culling stripelets, the remaining stripelets are then fed through the crosswalk detection algorithm. The results again are categorized by type of crosswalk, time of day, etc. 
%Show outlier data first - DOWN first, then Yellow, then the bad video

There were five different angles used for the videos of the crosswalks, with 340 frames of video at each angle. One of the angles had much worse results than the others, as shown in table \ref{tab:downbad}. The reason for this is that the cell phone camera was angled too low to capture enough stripelets for a positive identification, a few examples are shown in figure \ref{fig:downBadPic}. The results from the bad video angle tell us that in a real world application, that angle will be disallowed. The user could be trained to hold the cell phone camera at the proper angle, and if they did not, the sensors would allow the application to notify the user to adjust the camera angle. Because of this, the videos taken at that downward angle can be removed from the overall dataset. 
\todo[inline]{include a picture that shows all the angles on a single crosswalk}
\todo[inline]{I know first person isn't allowed, help me here, Jenee}
\todo[inline]{grab pic for down}
...
In order to give a glimpse at how yellow crosswalks worked with white crosswalk training data and this approach in general, two yellow crosswalks were tested, one being a standard zebra and the other a hybrid. The hybrid scored 26\% true positive rate, and the zebra scored 67.5\%, both out of 80 frames. Those results are promising enough to conclude that yellow crosswalks will likely work well with this approach, and would probably improve with yellow crosswalks being included in the training data. The yellow crosswalks will be removed from the dataset for the remainder of this section though, because it was just an experiment.
\todo[inline]{horribly worded jeneeplz}

\todo[inline]{time of day data}
Each crosswalk was categorized by time of day as well to see if that had any effect on the prediction rates, as shown in table \ref{tab:timeofday}. There were 300 crosswalk frames for each time of day. The results for each time of day were rather close together, with a slight decrease in the true positive rate from morning to night. This might have to do with the lighting being more difficult to categorize, or potentially just an outlier in the data (for example, the worst crosswalk in the dataset was a night crosswalk as discussed later on). 
\todo[inline]{finish}
...
\todo[inline]{per crosswalk data, with appendix for non-neural big chart}
Results were aggregated for each crosswalk (or non-crosswalk video) as well. The results of that is shown in the appendix in tables \ref{tab:appendixcrosswalkresults} and \ref{tab:appendixcrosswalkresultswithoutNeural}, and the summary results are shown in table \ref{tab:crosswalkResultsSummary}, separated into the crosswalks (which have true positive rates) and non-crosswalk videos (which have false positive rates).  There were a couple outliers in each category, to be looked into. 
...
\todo[inline]{speak to a couple of these, including the bad one}
...
\todo[inline]{explain some more about why it failed after grabbing pictures}
...
\todo[inline]{discuss more about this after grabbing pictures}
...
\todo[inline]{discuss more after getting photos. What are my guesses?}
    
\todo[inline]{overall data discussion}
